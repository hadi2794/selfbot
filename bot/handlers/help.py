...
{PREFIX}هوش <جمله‌ی آزاد> — یه جمله‌ی محاوره‌ای می‌گیره و تبدیلش می‌کنه به یادآوری (فعلاً فقط همین intent پشتیبانی می‌شه)
   مثال: `{PREFIX}هوش فردا ساعت ۹ یادم بنداز به علی زنگ بزنم`
{PREFIX}هوش تایید — ثبتِ یادآوریِ پیشنهادی
{PREFIX}هوش لغو — لغوِ پیشنهاد
   قبل از هر ثبتی خلاصه‌ی تشخیص‌داده‌شده رو نشون می‌ده و منتظرِ تاییدِ صریح می‌مونه؛ نیازمندِ همون AI_API_KEY بالا
